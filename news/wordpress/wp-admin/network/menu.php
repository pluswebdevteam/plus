...
$menu[2] = array(__('Dashboard'), 'manage_network', '/', '', 'menu-top menu-top-first menu-icon-dashboard', 'menu-dashboard', 'dashicons-dashboard');

$submenu['/'][0] = array(__('Home'), 'read', '/');
...
if ($update_data['counts']['total']) {
    $submenu['/'][10] = array(sprintf(__('Updates %s'), "<span class='update-plugins count-{$update_data['counts']['total']}'><span class='update-count'>".number_format_i18n($update_data['counts']['total']).'</span></span>'), $cap, 'update-core.php');
} else {
    $submenu['/'][10] = array(__('Updates'), $cap, 'update-core.php');
}

unset($cap);

$submenu['/'][15] = array(__('Upgrade Network'), 'upgrade_network', 'upgrade.php');
