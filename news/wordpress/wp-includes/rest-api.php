 ...
 *
 *     @var string|array|null $get_callback    Optional. The callback function used to retrieve the field
 *                                              value. Default is 'null', the field will not be returned in
 *                                              the response.
 *     @var string|array|null $update_callback Optional. The callback function used to set and update the
 *                                              field value. Default is 'null', the value cannot be set or
 *                                              updated.
 *     @var string|array|null $schema          Optional. The callback function used to create the schema for
...
function rest_api_register_rewrites()
{
    ...
    add_rewrite_rule('^'.rest_get_url_prefix().'/?$', '/?rest_route=/', 'top');
    add_rewrite_rule('^'.rest_get_url_prefix().'/(.*)?', '/?rest_route=/$matches[1]', 'top');
    add_rewrite_rule('^'.$wp_rewrite->index.'/'.rest_get_url_prefix().'/?$', '/?rest_route=/', 'top');
    add_rewrite_rule('^'.$wp_rewrite->index.'/'.rest_get_url_prefix().'/(.*)?', '/?rest_route=/$matches[1]', 'top');
}
...
function get_rest_url($blog_id = null, $path = '/', $scheme = 'rest')
{
    ...
    if (is_multisite() && get_blog_option($blog_id, 'permalink_structure') || get_option('permalink_structure')) {
        ...
        // To work around this, we manually add / to the URL, avoiding the redirect.
        if ('/' !== substr($url, 9)) {
            $url .= '/';
        }
